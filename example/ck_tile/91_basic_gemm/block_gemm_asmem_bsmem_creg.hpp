...
struct BlockGemmASmemBSmemCReg
{
                                   ...
                                   const BBlockWindowTmp& b_block_window_tmp) const
    {
        ...
        constexpr index_t MWarp = config.template get(number<1>{});      // 4
        constexpr index_t NWarp = config.template get(number<2>{});      // 1

        constexpr index_t MIterPerWarp = MPerBlock / (MWarp * WG::kM);   // 256 / (4 * 32) = 2
        constexpr index_t NIterPerWarp = NPerBlock / (NWarp * WG::kN);   // 128 / (1 * 32) = 4
        constexpr index_t KIterPerWarp = KPerBlock / WG::kK;             // 32 / 16 = 2, 16 or 8? instruction related

        constexpr index_t MPerBlockPerIter = MPerBlock / MIterPerWarp;   // 256 / 2 = 128
        constexpr index_t NPerBlockPerIter = NPerBlock / NIterPerWarp;   // 128 / 4 = 32
        constexpr index_t KPerBlockPerIter = KPerBlock / KIterPerWarp;   // 32 / 2 = 16

        const index_t iMWarp = get_warp_id() / NWarp;                    // 0, 1, 2, 3
        const index_t iNWarp = get_warp_id() % NWarp;                    // 0, 0, 0, 0
                                   ...
                                   const BBlockWindowTmp& b_block_window_tmp) const
    {
        ...
        constexpr index_t MIterPerWarp = MPerBlock / (MWarp * WG::kM);   // 256 / (4 * 32) = 2
        constexpr index_t NIterPerWarp = NPerBlock / (NWarp * WG::kN);   // 128 / (1 * 32) = 4
        constexpr index_t KIterPerWarp = KPerBlock / WG::kK;             // 32 / 8(16) = 4(2), 16 or 8? instruction related

        constexpr index_t MPerBlockPerIter = MPerBlock / MIterPerWarp;   // 256 / 2 = 128
        constexpr index_t NPerBlockPerIter = NPerBlock / NIterPerWarp;   // 128 / 4 = 32
        constexpr index_t KPerBlockPerIter = KPerBlock / KIterPerWarp;   // 32 / 4(2) = 8(16)
